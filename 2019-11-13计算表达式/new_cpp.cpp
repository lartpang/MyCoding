 ...
 * */
bool StackExprChecker::IsCharValid(const string &in_string) {
 ...
 * */
bool StackExprChecker::IsOperatorValid(string in_string) {
 ...
 * */
bool StackExprChecker::IsFloatValid(const vector<string> &in_string) {
  int pos;
 ...
 * */
vector<string> StackExprChecker::NegativeChecker(string in_string) {
  // -a+b和b+(-a)这样的
  ...
  for (int i = 0; i < in_string.size(); ++i) {
    ...
    if (i == 0) {
      ...
      if (IsOperator(in_string[i])) {
        ...
        if (c_stack.top() == '(' || IsOperator(in_string[i]) ||
            IsPair(in_string[i])) {
          // ()或者前为运算符或点号，非法
          return false;
        } else {
